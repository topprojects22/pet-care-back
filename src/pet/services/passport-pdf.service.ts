...
export class PassportPdfService {
  ...
  /**
   * Генерирует PDF паспорт питомца
   */
  async generatePassportPdf(
    ...
    // Настраиваем заголовки для скачивания PDF
    const filename = `pet-passport-${petId}.pdf`;
    ...
    if (pet.petPassport.breed) {
      doc.text(`Порода: ${pet.petPassport.breed.name}`, { indent: 20 });
    }
    if (pet.birthDate) {
      doc.text(
    ...
    doc.fontSize(12).font('Helvetica');
    doc.text(`Чип: ${pet.petPassport.chip}`, { indent: 20 });
    if (pet.petPassport.chipInstallDate) {
      doc.text(
        `Дата установки чипа: ${new Date(pet.petPassport.chipInstallDate).toLocaleDateString('ru-RU')}`,
        { indent: 20 },
      );
    }
    if (pet.petPassport.tattooNumber) {
      doc.text(`Татуировка: ${pet.petPassport.tattooNumber}`, { indent: 20 });
    }
    if (pet.petPassport.registrationNumber) {
      doc.text(
        `Регистрационный номер: ${pet.petPassport.registrationNumber}`,
        { indent: 20 },
      );
    }
    if (pet.petPassport.issuingOrganization) {
      doc.text(
        `Организация, выдавшая паспорт: ${pet.petPassport.issuingOrganization}`,
        { indent: 20 },
      );
    }
    if (pet.petPassport.specialMarks) {
      doc.text(`Особые приметы: ${pet.petPassport.specialMarks}`, {
        indent: 20,
      });
    }
    doc.moveDown();
